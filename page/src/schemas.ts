...
const books = `
The “Books” schema is a fully-fledged app example schema and allows you to query books & writers.
`
  .trim()
  .split('\n');
  ...
  if (!client) {
    ...
    client = createClient({
      ...
      exchanges: [
        ...
        schema === 'web-collections' &&
          authExchange<{ current: null | Tokens }>({
            async getAuth({ authState, mutate }) {
              if (!authState) {
                ...
                if (refreshCredentials) {
                  ...
                  authState.current.refreshToken =
        ...
        schema === 'web-collections' &&
          resultExchange({
            onResult(result) {
              const credentials =
                result.data?.signin || result.data?.register || {};
              if (
                credentials.__typename === 'Credentials' &&
                credentials.token &&
                credentials.refreshToken
              ) {
                tokensRef.current = Object.assign(
                  tokensRef.current || {},
                  credentials
                );
              }
        ...
        retryExchange({
          ...
          retryIf:
            schema === 'intermittent-colors'
              ? (error) =>
                  error.graphQLErrors.some(
                    (x) => x.extensions?.code === 'NO_SOUP'
                  ) || !!error.networkError
